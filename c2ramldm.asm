...
; imidiate write configrations
	...
	ld	a,#20			; immediate changes enabled
	ld	(CardMDR),a
	ld	a,4			;start block (absolute block 64kB), 4 for RAM/Flash
	ld	(AddrFR),a

	; write bank config from Record
	ld	hl,Record+#23
	ld	de,R1Mask
	ld	bc,6*4			; bank_reg*4
	ldir				; write bank setting
	
	ld	a,(hl)			; Mconf
	inc	hl
;	ld	(Mconf),a		; Ignore setting - Stay current setting

	ld	a,(hl)			; Record's CardMDRCopy
;	inc	hl
	and	%00010000		; scc flag
	or 	%00000000		; immediate changes enabled / config reg=#0F80
	ld	(CardMDR),a
...
;---
	...
	or	a			; change now?
	jr	z,DoTest
	; force write register (reset config)
	
	ld	hl,CardMDR-#4000	;CardMDR placed #0F80
	ld	e,#20			; immediate changes enabled / config reg. = #4F80
	ld	a,d			; slot no.
	call	WRTSLT			; inter slot write byte
	
	ld	a,#20			; immediate changes enabled
	ld	(CardMDR),a
	xor	a			; start block (absolute block 64kB), 4 for RAM/Flash
	ld	(AddrFR),a
	ld	hl,RSTCFG		; reset config
	ld	de,R1Mask
	ld	bc,26
	ldir
...
fkey07:
	...
	jr	nz,fkey08
	ld	a,7
	ld	(F_E),a			; enable ROM and exit after loading ROM
	ret
fkey08:
	ld	hl,BUFFER+1
	ld	a,(hl)
...
Det06:	db	0
;Det04:	ds	134
DMAP:	db	0	; Detected Mapper Type
DMAPt:	db	1
BMAP:	ds	2
Dpoint:	db	0,0,0
StartBL:
...
MAIN_S:	db	13,10
	...
	db	" 4 - Enable writed ROM image, and exit to MSX-DOS",13,10
...
Prg_Su_S:
	db	13,10,"The ROM image was successfully written into cartridge's RAM!",13,10,"$"
Prg_Su2_S:
	db	"Please reboot (NO POWER OFF!) your MSX now...",13,10,"$"
FL_er_S:
...
H_PAR_S:
	...
	db	" c2ramldr [filename.rom] [/h] [/v] [/a] [/p] [/d] [/r] [/e] [/f]",13,10,13,10
	...
	db	" /e  - enable ROM image and exit after uploading the ROM",13,10
	db	" /f  - force detect slot(force write reset config)",13,10
	db	"$"
